  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(findThirdUser.fulfilled, (state, { payload: users }) => {
      ...
      const totalAvailablePages = Math.max(
        Math.floor(users.total_count / divisor),
        1,
      );

      state.globalTotal = users.total_count;
      state.globalTotalPageAvailable =
...
export const { unsetUser, clearGlobalUsers, pageUp, pageOne, pageDown } =
  usersSlice.actions;
export const selectUser = (state: RootState) => state.users.user;
export const selectGlobalUsers = (state: RootState) => state.users.globalUsers;
export const selectGlobalUserPage = (state: RootState) =>
  state.users.globalUserPage;
export const selectGlobalTotal = (state: RootState) => state.users.globalTotal;
export const selectGlobalTotalPage = (state: RootState) =>
  state.users.globalTotalPageAvailable;
export const selectAuthLoading = (state: RootState) => state.users.loading;
